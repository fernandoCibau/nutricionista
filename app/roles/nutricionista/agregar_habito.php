...
if ($id_paciente === false || empty($descripcion)) {
...
if (!$check || $check->rowCount() === 0) {
    header('Location: index.php?error=tabla_habitos_no_exist');
    exit;
}
...
try {
    $ins = $pdo->prepare("INSERT INTO habitos (id_paciente, id_nutricionista, descripcion, creado_en) VALUES (?, ?, ?, NOW())");
    $ins->execute([$id_paciente, $_SESSION['user_id'], $descripcion]);
    header('Location: index.php?exito=habito_agregado');
    exit;
} catch (PDOException $e) {
    error_log('Error al agregar hábito: ' . $e->getMessage());
