...
try {
    ...
    $q = $pdo->prepare("SELECT id, nombre_archivo, url_archivo, fecha_subida FROM archivos_plan WHERE id_paciente=? ORDER BY fecha_subida DESC");
    $q->execute([$pacienteId]);
    $rows = $q->fetchAll();
    echo json_encode(['success' => true, 'data' => $rows]);
} catch (Throwable $e) {
    error_log('archivos_plan_listar: '.$e->getMessage());
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Error al listar archivos']);
}
