...
try {
    ...
    $q = $pdo->prepare("SELECT id, fecha, peso_kg, altura_cm, masa_muscular_pct, comentarios, habitos_cumplidos, objetivos_trabajados FROM consultas WHERE id_paciente=? ORDER BY fecha DESC");
    $q->execute([$pacienteId]);
    $rows = $q->fetchAll();
    echo json_encode(['success' => true, 'data' => $rows]);
} catch (Throwable $e) {
    error_log('consultas_listar: '.$e->getMessage());
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Error al listar consultas']);
}
