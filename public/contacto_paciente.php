<?php
    ...
    try {
        $stmt = $pdo->query("
            SELECT u.id, u.nombre, d.localidad
            FROM usuarios u
            JOIN nutricionistas n ON u.id = n.id_usuario
            JOIN nutricionista_direcciones d ON n.id = d.id_nutricionista
            WHERE u.role_id = 2
            ORDER BY d.localidad, u.nombre
        ");
        $nutricionistas = $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (PDOException $e) {
        // Manejo básico de errores
        error_log("Error al obtener nutricionistas: " . $e->getMessage());
        $nutricionistas = []; // Asegurarse de que la variable exista
    }
?>
<!DOCTYPE html>
<html lang="es">
        ...
        <div class="row g-5">
            ...
            <div class="col-md-6">
                <form action="enviar_contacto.php" method="POST">
                    ...
                    <div class="mb-3">
                        <label for="nutricionista" class="form-label">Profesional más cercano</label>
                        <select class="form-select" id="nutricionista" name="id_nutricionista">
                            <option value="">Selecciona un profesional</option>
                            <?php foreach ($nutricionistas as $nutri): ?>
                                <option value="<?php echo htmlspecialchars($nutri['id']); ?>">
                                    <?php echo htmlspecialchars($nutri['nombre'] . ' - ' . $nutri['localidad']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
